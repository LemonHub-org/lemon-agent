...
An unattended, long-running autonomous programming agent built in Rust — 使用 Rust 构建的、可长期无人值守运行的自主编程 Agent。

It plans, edits code, runs commands, tests, verifies, and evolves its own Rhai
strategy scripts within a safe sandbox. 它在安全沙箱内完成规划、代码修改、
命令执行、测试、验证，并自主进化自己的 Rhai 策略脚本。
...
Status: v1.0 implemented. 状态：v1.0 已实现。
