...
- **进化范围**: 仅限于脚本层（Rhai 脚本）的热更新，不涉及 Rust 内核的自我修改
- **运行环境**: 跨平台
### 1.3 非目标
...
└──────────────┬──────────────────────────┘
               ...
               ▼
┌─────────────────────────────────────────┐
│         能力层 (Capability Layer)        │  ← Rust 核心，安全边界
...
└──────────────┬──────────────────────────┘
               ...
               ▼
┌─────────────────────────────────────────┐
│         脚本引擎 (Script Engine)         │  ← Rhai 运行时，可进化
│  - 动态加载 scripts/*.rhai              │
│  - 暴露 Rust 工具为 Rhai 函数           │
│  - 热重载 (hot-reload)                  │
└──────────────┬──────────────────────────┘
...
### 2.2 数据存储
- **SQLite**: 单一数据库文件 `agent.db`，用于事件溯源和状态快照。
- **文件系统**: 项目代码存储在工作目录；脚本存储在 `scripts/` 子目录。
...
**恢复流程**:
1. 从 `snapshots` 读取最新快照（按 `seq` 降序）。
2. 从 `events` 中读取该 `seq` 之后的所有事件。
3. 按顺序重放事件，重建内存状态。
...
### 3.3 上下文管理 (Context Manager)

**文件**: `src/scheduler/context.rs`

管理 LLM 的对话上下文，采用**滑动窗口**保留最近 `K` 条消息，并结合摘要压缩。

- 数据结构: `Vec<Message>`，每条消息包括 `role` (system/user/assistant/tool) 和 `content`。
- 限制: 总 token 数不得超过 `MAX_CONTEXT_TOKENS` (如 128k)。
- 压缩策略: 当 token 数超过阈值时，调用 LLM 生成之前对话的摘要，替换为一条 system 消息。
...
```rust
pub struct Budget {
    pub max_steps: usize,           // 默认为 200
    pub max_input_tokens: usize,    // 默认为 100_000
    pub max_llm_calls: usize,       // 默认为 50
    pub max_tool_calls: usize,      // 默认为 200
    pub max_wall_clock_secs: u64,   // 默认为 86400 (24小时)
}
```

检查点: 每个步骤开始前调用 `check_budget()`，如果超限则触发 `Terminated` 状态并记录原因。

### 3.5 能力与沙箱 (Capability & Sandbox)

**文件**: `src/kernel/capability.rs`, `src/kernel/sandbox.rs`

#### 权限令牌

每个工具调用都需要提供有效的权限令牌。令牌结构:

```rust
pub struct Capability {
    pub resource: Resource,        // 如 "fs:/project", "process:allowed_commands"
    pub permissions: Permissions,  // 读/写/执行
...
pub struct Sandbox {
    ...
    allowed_commands: HashSet<String>, // 如 ["git", "cargo", "python"]
}
impl Sandbox {
...
封装 HTTP 请求，支持 OpenAI 兼容 API（如 DeepSeek, GPT-4）。

- 使用 `reqwest` 异步客户端，带重试机制（指数退避，最多3次）。
- 支持流式响应（可选），以便实时输出。
- 支持工具调用（Function Calling）: 将 Rust 工具转换为 JSON Schema。
...
基于 `Rhai` 嵌入式脚本语言，动态加载 `scripts/` 目录下的 `.rhai` 文件。

**暴露给脚本的 Rust 函数**（通过 `register_fn`）：

```rust
// 所有工具函数都包装为 Rhai 可调用的
...
- 启动时扫描 `scripts/` 下所有 `.rhai` 文件，编译为 `AST` 并缓存。
- 热重载: 使用 `notify` 库监听文件变化，自动重新编译更新。
...
1. **捕获失败上下文**: 收集错误消息、当前脚本源码、最近N条对话消息、相关事件。
...
7. **记录事件**: 生成 `EvolutionAttempt` 和 `EvolutionResult` 事件。
...
所有工具都通过 `Sandbox` 实现，并以 Rhai 函数形式暴露。

| 工具名称 | Rhai 签名 | 安全限制 | 备注 |
|---------|----------|---------|------|
| `read_file` | `fn read_file(path: String) -> Result<String>` | 路径必须在 `root_dir` 内；文件大小<10MB | 异步 |
...
| `exec_command` | `fn exec_command(cmd: String, args: Array) -> Result<Object>` | 命令白名单；禁止交互；超时120秒 | 异步 |
| `git_add_commit` | `fn git_add_commit(message: String) -> Result<String>` | 必须为Git仓库；限制分支 | 异步 |
| `llm_query` | `fn llm_query(prompt: String) -> Result<String>` | 受预算限制 | 异步 |
| `sleep` | `fn sleep(ms: int) -> Result<()>` | 用于延迟 | 异步 |
| `log_debug` | `fn log_debug(msg: String)` | 仅日志 | 同步 |

**注意**: 所有工具执行结果都将被记录为事件 `ToolCall`，方便审计。
...
5. **Evolving**: 如果验证失败，进入进化流程，修改相关脚本，然后重复执行步骤3-4。
6. **Termination**: 验证通过或预算耗尽，生成最终报告，保存快照，进入 `Idle` 等待新任务。
...
- **系统级**: 可通过 systemd 或 Docker 重启策略监控进程存活。

### 6.2 恢复机制
- 启动时自动从 SQLite 恢复最近快照+事件。
- 若连续性 ID 存在且未完成，则继续执行；否则创建新连续性。
...
4. **备份**: 将 `scripts/<script_name>.rhai` 重命名为 `<script_name>.rhai.bak`。
5. **写入新脚本**: 原子写入新内容。
6. **重载**: 通知脚本引擎重新编译该脚本。
7. **验证**: 使用 `sandbox` 调用新脚本的一个测试入口（如 `test_<script_name>()`）或重跑失败步骤。
8. **结果处理**:
   - 成功：保留新脚本，记录 `EvolutionResult{success: true}`。
   - 失败：恢复备份，记录 `EvolutionResult{success: false, reason}`，并可能降低该脚本的“信任分数”。
9. **限制**: 每个连续性最多进行 `MAX_EVOLUTION_ATTEMPTS` (默认5) 次进化尝试。
...
### 9.3 Docker 镜像
提供 Dockerfile，使用 `rust:alpine` 构建，复制二进制和脚本目录。

### 9.4 日志监控
使用 `tracing` 输出结构化日志，可集成 `tracing-subscriber` 写入文件或 stdout。
...
### 10.1 当前限制
- 仅支持单线程异步执行。
- 进化仅限于脚本层，无法修改 Rust 内核。
- 没有多项目隔离，所有任务共享工作目录。
- 依赖 LLM 的稳定性，可能因API限流或错误而中断。

### 10.2 未来计划
- **分布式执行**: 使用 `tokio` 多线程，支持并行子任务。
- **更丰富的工具**: 支持数据库查询、网络请求、Docker操作。
- **深度学习辅助**: 基于历史事件微调提示或选择最优脚本。
- **自我测试**: 自动生成测试套件并运行。
- **Rust 源码级进化（探索性）**: 在完全受控的实验中，让 Agent 生成 Rust 代码并通过热加载动态库（`libloading`）实现部分功能更新。
